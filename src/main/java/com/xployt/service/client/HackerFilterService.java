...
public class HackerFilterService {
    ...
    private final Set<String> redundantWords = new HashSet<>(Arrays.asList("Security", "Modules", "Processing", "Transfer",
            "Storage", "Gateway", "Compliance", "Data", "User"));
    ...
    public GenericResponse filterHackers(String projectId) throws SQLException {
        ...
        List<String> filteredHackers = filterInvitedHackers(hackerIds, projectId);
    ...
    private List<Hacker> selectHackersWithSpreadBlastPoints(List<Hacker> hackers) {
        ...
        Set<Hacker> uniqueHackers = new HashSet<>(selectedHackers);
        while (uniqueHackers.size() < MAX_SELECTED_HACKERS) {
            Hacker randomHacker = hackers.get(random.nextInt(hackers.size()));
            uniqueHackers.add(randomHacker);
        }
