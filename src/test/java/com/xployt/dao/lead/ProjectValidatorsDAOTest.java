...
// TEST_CLIENT_ID = SetupUsers.makeTestClient("Test Client",
// "test.client@test.com", conn);
// TEST_PROJECT_ID = SetupProject.makeTestProject(TEST_CLIENT_ID, "Test
// Project", conn);
// SetupProject.addProjectScopeRandom(TEST_PROJECT_ID, 7, conn);
// TEST_PROJECT_LEAD_ID = SetupUsers.makeProjectLead("Test Project Lead",
// "test.projectlead@test.com", conn);
// TEST_VALIDATOR_ID_1 = SetupUsers.makeTestValidators("Test Validator 1",
// "test.validator1@test.com", conn);
// TEST_VALIDATOR_ID_2 = SetupUsers.makeTestValidators("Test Validator 2",
// "test.validator2@test.com", conn);
...
// projectValidatorsDAO.assignValidatorsBasedOnSkills(TEST_PROJECT_ID,
// validatorCount, conn);
...
// stmt.execute("DELETE FROM ValidatorSkillSet WHERE validatorId IN (" +
// TEST_VALIDATOR_ID_1 + ", "
// + TEST_VALIDATOR_ID_2 + ")");
// stmt.execute("DELETE FROM ProjectValidators WHERE projectId IN (" +
// TEST_PROJECT_ID + ")");
// stmt.execute(
// "DELETE FROM ValidatorInfo WHERE validatorId IN (" + TEST_VALIDATOR_ID_1 + ",
// " + TEST_VALIDATOR_ID_2
// + ")");
// stmt.execute(
// "DELETE FROM ValidatorInfo WHERE validatorId IN (" + TEST_VALIDATOR_ID_1 + ",
// " + TEST_VALIDATOR_ID_2
// + ")");
// stmt.execute("DELETE FROM ProjectScope WHERE projectId IN (" +
// TEST_PROJECT_ID + ")");
// stmt.execute("DELETE FROM ProjectConfigs WHERE projectId IN (" +
// TEST_PROJECT_ID + ")");
...
// stmt.execute("DELETE FROM Users WHERE userId IN (" + TEST_VALIDATOR_ID_1 + ",
// " + TEST_VALIDATOR_ID_2 + ")");
